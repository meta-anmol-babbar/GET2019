...
import com.metacube.eadsession9and10.mapper.EmployeeInfoMapper;
import com.metacube.eadsession9and10.mapper.EmployeeMapper;
import com.metacube.eadsession9and10.mapper.FriendsExtractor;
import com.metacube.eadsession9and10.mapper.ProfileMapper;
...
public class ApplicationDAO {
	...
	public List<String> getFriends(String email) {
		return jdbcTemplate.query(SQL_GET_FRIENDS, new Object[] { email },
				new FriendsExtractor());
	}
	...
	public boolean addVehicle(int empId, String name, String type,
		...
		int row = jdbcTemplate.update(new PreparedStatementCreator() {
			...
			public PreparedStatement createPreparedStatement(
					Connection connection) throws SQLException {
				PreparedStatement ps = connection
						.prepareStatement(SQL_INSERT_VEHICLE);
				System.out.println(empId + name + type + vehicleNumber
						+ identification);
				ps.setInt(1, empId);
				ps.setString(2, name);
				ps.setString(5, type);
	...
	public boolean addPlan(int empId, String vehicleNumber, String plan,
		...
		int row = jdbcTemplate.update(new PreparedStatementCreator() {
			...
			public PreparedStatement createPreparedStatement(
					Connection connection) throws SQLException {
				PreparedStatement ps = connection
						.prepareStatement(SQL_INSERT_PLAN);
				ps.setInt(1, empId);
				ps.setString(3, plan);
	...
	public ProfileCommands getProfile(String email) {
		return jdbcTemplate.queryForObject(SQL_FIND_PROFILE,
				new Object[] { email }, new ProfileMapper());
	}

	public EmployeeInfo getEmpoyeeInfo(String email, String password) {
		try {

			return jdbcTemplate.queryForObject(SQL_FIND_INFO, new Object[] {
					email, password }, new EmployeeInfoMapper());
		} catch (EmptyResultDataAccessException e) {
			return null;
		}
	...
	public boolean uploadPhoto(String email, String imgName) {
		int row = jdbcTemplate.update(new PreparedStatementCreator() {
			...
			public PreparedStatement createPreparedStatement(
					Connection connection) throws SQLException {
				PreparedStatement ps = connection
						.prepareStatement(SQL_UPLOAD_IMAGE);
	...
	public boolean updateUploadPhoto(String email, String imgName) {
		int row = jdbcTemplate.update(new PreparedStatementCreator() {
			...
			public PreparedStatement createPreparedStatement(
					Connection connection) throws SQLException {
				PreparedStatement ps = connection
						.prepareStatement(SQL_UPDATE_IMAGE);
