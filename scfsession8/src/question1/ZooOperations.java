...
public class ZooOperations {
	...
	public void display() {
		...
		for (int i = 0; i < zoneList.size(); i++) {
			System.out.println("zone number : " + (i + 1) + "\t" + " hasPark :"
					+ zoneList.get(i).hasPark() + "\t" + " hascanteen:"
					+ zoneList.get(i).hasCanteen() + "\n");
			for (int j = 0; j < zoneList.get(i).cages.size(); j++) {
				...
				for (int k = 0; k < zoneList.get(i).cages.get(j).animals.size(); k++)
					System.out.println(" id : "
							+ zoneList.get(i).cages.get(j).animals.get(k).aid
							+ "\t"
							+ " name : "
							+ zoneList.get(i).cages.get(j).animals.get(k).name
							+ "\t"
							+ " sound : "
							+ zoneList.get(i).cages.get(j).animals.get(k)
									.getSound());
			}
			System.out.println("\n\n");
		}
	 ...
	 * 
	 * @param category
	 *            is the category of animal to be added
	 * @param type
	 *            of animal to be added in zoo
	 */
	public void addAnimal(String category, String type) {
		...
		for (int i = 0; i < zoneList.size(); i++) {
			...
			if (zoneList.get(i).category.equals(category)) {
				...
				for (j = 0; j < zoneList.get(i).cages.size(); j++) {
					// checking if space is present in type of cage and then
					// adding
					if (zoneList.get(i).cages.get(j).type.equals(type)
							&& zoneList.get(i).cages.get(j).animals.size() < zoneList
									.get(i).cages.get(j).capacity) {
						if (type.equals("lion"))
							zoneList.get(i).cages.get(j).animals.add(new Lion(
									name, age, weight));
						if (type.equals("crocodile"))
							zoneList.get(i).cages.get(j).animals
									.add(new Crocodile(name, age, weight));
						if (type.equals("peacock"))
							zoneList.get(i).cages.get(j).animals
									.add(new Peacock(name, age, weight));
				...
				if (zoneList.get(i).cages.size() < zoneList.get(i).noOfCages) {
					System.out
							.println("Creating new cage .. \nenter the capacity of cage : ");
					capacity = sc.nextInt();
					zoneList.get(i).cages.add(new Cage(type, capacity));
					if (type.equals("lion"))
						zoneList.get(i).cages.get(j).animals.add(new Lion(name,
								age, weight));
					if (type.equals("crocodile"))
						zoneList.get(i).cages.get(j).animals.add(new Crocodile(
								name, age, weight));
					if (type.equals("peacock"))
						zoneList.get(i).cages.get(j).animals.add(new Peacock(
								name, age, weight));
					System.out.println("animal added successfully..\n ");
					return;
				}
		...
		System.out
				.println("No space available in any zone.. pls create a new zone.. ");
	 ...
	 * 
	 * @param name
	 *            is the name of animal to be removed
	 */
	public void removeAnimal(String name) {
		...
		for (int i = 0; i < zoneList.size(); i++) {
			for (int j = 0; j < zoneList.get(i).cages.size(); j++) {
				for (int k = 0; k < zoneList.get(i).cages.get(j).animals.size(); k++)
					if (zoneList.get(i).cages.get(j).animals.get(k).name
							.equals(name)) {
