...
FROM
    ...
    user
WHERE
    shopperorder.date >= NOW()- INTERVAL 15 DAY AND shopperorder.cart_id = cart_list.cart_id AND cart_list.user_id = user.user_id AND cart_list.product_id = products.product_id
