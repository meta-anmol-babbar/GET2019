...
/**
 * This class contains methods to convert infix expression to postfix expression
...
public class ExpressionEvaluation {
	...
	String infixToPostfix() {
				...
				if (")".equals(infixArray[i])) {
					...
					if (isOperator(infixArray[i])) {
						if (stack.isEmpty()) {
							...
							if (((getPrecedence(infixArray[i])) > (getPrecedence(stack
									.peek())))) {
								stack.push(infixArray[i]);
							} else {
								while ((getPrecedence(infixArray[i]) <= getPrecedence(stack
										.peek()))
										&& !("(".equals(stack.peek()))) {
									postfixExpression.append(stack.pop());
									postfixExpression.append(" ");
		...
		// Append the remaining operators from the stack to the postfix
		// expression
		while (!(stack.isEmpty())) {
			postfixExpression.append(stack.pop());
			postfixExpression.append(" ");
	 ...
	 * 
	 * @param operator
	 *            is the operator whose precedence is to be determined
